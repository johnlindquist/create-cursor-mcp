...
try {
    ...
    const cli = execSync(`node ${cliPath} --name ${projectName} --skip-deploy`, {
        stdio: 'inherit',
        env: {
            ...process.env,
    ...
    if (existsSync(indexPath)) {
        ...
        const hasTemplateCode = content.includes('MCPMathServer') &&
            (content.includes('add') && content.includes('subtract'));

        if (hasTemplateCode) {
            console.log(pc.green('✅ src/index.ts contains the template code'));
        } else {
            console.error(pc.red('❌ src/index.ts does not contain the expected template code'));
    ...
    console.log(pc.yellow(`To prevent cleanup and inspect the files, press Ctrl+C now.`));
